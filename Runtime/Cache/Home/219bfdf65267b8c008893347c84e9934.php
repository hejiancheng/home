...
<meta charset="UTF-8">
<title>试卷页面</title>
<link rel="stylesheet" type="text/css" href="/Hlt/Public/css/bootstrap-3.3.4.css">
<link href="/Hlt/Public/css/index.css" rel="stylesheet" type="text/css"/>
<script src="/Hlt/Public/js/jquery-1.9.1.min.js"></script>
<script type="text/javascript" src="/Hlt/Public/js/jquery.min.js"></script>
<script src="/Hlt/Public/js/bootstrap.js"></script>
<script>
var HH = 0;//时
var mm = 0;//分
var ss = 0;//秒
...
/*实现计时器*/
 ...
 function init(){
	 var sure=confirm("开始刷题！中途退出视同放弃");
 	if(!sure){
	 ...
	 $.ajax({   
        type:"post",     
        url:"<?php echo U('UsersTopic/get_topic');?>", 
        dataType:"json",
...
function showQuestion(id){
    ...
    for(var i=0;i<items.length;i++){
        item ="<li class='question_info' style='margin-bottom:50px;margin-top:50px' onclick='clickTrim(this)' id='item"
                +i+"'><input type='checkbox' class='classitem"+i+"' name='item' value='"+itemList[i]+"'>&nbsp;"+itemList[i]+"."+items[i]+"</li>";
        $(".question").append(item);
    }
    $(".question").attr("id","question"+id);
    $("#ques"+id).removeClass("active_question_id").addClass("question_id");
	   if(checkAnswer[activeQuestion]!=undefined){
		   for(var i=0;i<7;i++){
		        if(checkAnswer[activeQuestion][i]==1){
		            $("#item"+i).find("input").prop("checked","checked");
...
function clickTrim(source){
	...
	if($(".class"+id).is(':checked')){
	    ...
	    var x=id.split("item");
	    checkAnswer[activeQuestion][x[1]]=0;
	}else{
		$(".class"+id).prop("checked","checked");
		 $("#"+id).addClass("clickTrim");
		 var x=id.split("item");
		 if(checkAnswer[activeQuestion] == undefined){
		 checkAnswer[activeQuestion]=new Array();
	     }
		 checkAnswer[activeQuestion][x[1]]=1;
	}
    $("#ques"+activeQuestion).removeClass("question_id").addClass("clickQue");
    var ques =0;
...
function init_show(){
	$(function(){
	    ...
	    $("#submitQuestions").click(function(){
	    	var sure=confirm("已做答:"+($(".clickQue").length)+"道题,还有"+(questions.length-($(".clickQue").length))+"道题未完成!\n\t\t确认提交?");
	    	if(!sure){
	    		return;
	    	}
	    	$.ajax({   
	            type:"post",     
	            url:"<?php echo U('UsersTopic/submit_answer');?>", 
	            dataType:"json",
	            data:{
	            	"id":num_top,
	            	"answer": checkAnswer
